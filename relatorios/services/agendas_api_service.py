...
class AgendasService:
    ...
    def buscar_agendas(self, processo_convocacao_uuid: str, page: int=1, page_size: int=100) -> requests.Response:
        ...
        logger.info('Buscando agendas', extra={'correlation_id': get_correlation_id(), 'method': 'GET', 'url': url, 'params': params, 'headers': self._default_headers})
        try:
            response = http_client.get(url, params=params, headers=self._default_headers, timeout=self.timeout_seconds)
            response.raise_for_status()
        except RequestException as exc:
            logger.error('Erro ao buscar agendas (processo_convocacao_uuid=%s): %s', processo_convocacao_uuid, exc)
            raise
        logger.info('Agendas buscadas com sucesso', extra={'correlation_id': get_correlation_id(), 'method': 'GET', 'url': url, 'params': params, 'headers': self._default_headers})
    ...
    def buscar_agenda_por_uuid(self, agenda_uuid: str) -> requests.Response:
        ...
        try:
            ...
            logger.error('Erro ao buscar agenda por UUID', extra={'correlation_id': get_correlation_id(), 'method': 'GET', 'url': url, 'headers': self._default_headers, 'error': str(exc)})
            raise
        logger.info('Agenda buscada com sucesso', extra={'correlation_id': get_correlation_id(), 'method': 'GET', 'url': url, 'headers': self._default_headers})
        return response  # type: ignore[no-any-return]
