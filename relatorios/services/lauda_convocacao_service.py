    ...
    def _buscar_candidatos_faltantes(self, outros_processos_uuid: list[str], lacunas_geral: list[int], lacunas_nna: list[int], lacunas_pcd: list[int], codigo_cargo: list[str] | str | None=None, ordering: str='ranking_escolha') -> dict[str, list[dict]]:
        ...
        try:
            if lacunas_geral:
                ...
                candidatos_uuids = [str(c.get('uuid')) for c in candidatos_faltantes['geral'] if c.get('uuid') is not None]
                reconvocacao_uuids: set[str] = set()
                if candidatos_uuids:
                    escolhas_reconvocacao = self.escolhas_service.buscar_escolhas_por_candidatos(candidato_uuids=candidatos_uuids, situacao='reconvocacao')
                    reconvocacao_uuids = set((str(e.get('candidato_uuid')) for e in escolhas_reconvocacao or [] if e.get('candidato_uuid') is not None))
                for candidato in candidatos_faltantes['geral']:
                    status_especial = 'JÁ CONVOCADO - LEI 13.398/02' if candidato.get('classificacao_nna') is not None else 'JÁ CONVOCADO - LEI 15.939/13' if candidato.get('classificacao_pcd') is not None else ''
                    if str(candidato.get('uuid')) in reconvocacao_uuids:
                        status_especial = f'{status_especial} - OPTOU POR RECONVOCAÇÃO' if status_especial else 'OPTOU POR RECONVOCAÇÃO'
            ...
            if lacunas_nna:
                ...
                candidatos_uuids = [str(c.get('uuid')) for c in candidatos_faltantes['nna'] if c.get('uuid') is not None]
                ...
                if candidatos_uuids:
                    try:
                        ...
                        logger.exception('Erro ao buscar escolhas (reconvocacao) para candidatos NNA faltantes', extra={'correlation_id': get_correlation_id(), 'candidatos_uuids_count': len(candidatos_uuids)})
            ...
            if lacunas_pcd:
                ...
                candidatos_uuids = [str(c.get('uuid')) for c in candidatos_faltantes['pcd'] if c.get('uuid') is not None]
                reconvocacao_uuids: set[str] = set()  # type: ignore[no-redef]
                if candidatos_uuids:
                    try:
                        escolhas_reconvocacao = self.escolhas_service.buscar_escolhas_por_candidatos(candidato_uuids=candidatos_uuids, situacao='reconvocacao')
                        ...
                        logger.exception('Erro ao buscar escolhas (reconvocacao) para candidatos PCD faltantes', extra={'correlation_id': get_correlation_id(), 'candidatos_uuids_count': len(candidatos_uuids)})
    ...
    def _inserir_reclassificados_em_segmento(self, lista_segmento: list[dict], reclassificados: list[dict], cargo_codigo: Any, classificacao_attr: str) -> None:
        ...
        try:
            if not isinstance(reclassificados, list) or not reclassificados or (not lista_segmento):
                return
            reclass_filtrados = [r for r in reclassificados if str(r.get('codigo_cargo')) == str(cargo_codigo) and isinstance(r.get(classificacao_attr), int)]
            ...
            while j < len(lista_segmento):
                ...
                if isinstance(cand_class, int):
                    candidatos_para_inserir = [r for r in reclass_filtrados if r.get(classificacao_attr) < cand_class and str(r.get('uuid')) not in existentes]  # type: ignore[operator]
                    candidatos_para_inserir.sort(key=lambda x: x.get(classificacao_attr))  # type: ignore[arg-type,return-value]
        ...
        try:
            ...
            while j < len(lista_segmento):
                ...
                for r in elim_filtrados:
                    if r.get('categoria_efetiva') == cand.get('categoria_efetiva') and (not cand.get('foi_reclassificado', False)) and (not r.get('foi_eliminado', False)) and (r.get(classificacao_attr) < cand_class) and (str(r.get('uuid')) not in existentes):  # type: ignore[operator]
        ...
        try:
            ...
            for agenda in agendas:
                ...
                if cargo_codigo not in agendas_por_cargo:
                    agendas_por_cargo[cargo_codigo] = {'cargo_nome': cargo_nome, 'cargo_codigo': cargo_codigo, 'agendas': []}
            ...
            response_reclassificados = self.candidatos_service.buscar_reclassificados_por_concurso(concurso_uuid=processo_data.get('concurso_uuid'), processo_uuid=processo_uuid)
            ...
            for cargo_info in agendas_por_cargo.values():
                ...
                for agenda in agendas_cargo:
                    response_candidatos = self.candidatos_service.buscar_por_uuids(uuids=agenda.get('candidatos_uuids'), order_by=ordering)
                    ...
                    if any((c.get('classificacao_pcd') is not None for c in candidatos_result)):
                        ...
                        classificacao_max = classificacao_max_default
                        classificacao_max_sem_pcd = 0
                    response_eliminados = self.candidatos_service.buscar_eliminados_por_concurso(concurso_uuid=processo_data.get('concurso_uuid'), processo_uuid=processo_uuid, classificacao_max=classificacao_max, classificacao_min=classificacao_min)
                    ...
                    self._inserir_reclassificados_em_segmento(lista_segmento=candidatos_base_ordenados, reclassificados=reclass_pcd, cargo_codigo=cargo_codigo, classificacao_attr='classificacao_pcd')
                    reclass_nna = (reclassificados_data or {}).get('nna', []) if reclassificados_data else []
                    self._inserir_reclassificados_em_segmento(lista_segmento=candidatos_base_ordenados, reclassificados=reclass_nna, cargo_codigo=cargo_codigo, classificacao_attr='classificacao_nna')
                    self._inserir_eliminados_em_segmento(lista_segmento=candidatos_base_ordenados, eliminados=(eliminados_data or {}).get('pcd', []) if eliminados_data else [], cargo_codigo=cargo_codigo, classificacao_attr='classificacao_pcd')
                    self._inserir_eliminados_em_segmento(lista_segmento=candidatos_base_ordenados, eliminados=(eliminados_data or {}).get('nna', []) if eliminados_data else [], cargo_codigo=cargo_codigo, classificacao_attr='classificacao_nna')
                    self._inserir_eliminados_em_segmento(lista_segmento=candidatos_base_ordenados, eliminados=(eliminados_data or {}).get('geral', []) if eliminados_data else [], cargo_codigo=cargo_codigo, classificacao_attr='classificacao')
                    numero_sessao = len(sessoes_cargo) + 1
                    agenda_sessao = agendas_cargo[numero_sessao - 1] if numero_sessao - 1 < len(agendas_cargo) else {}
                    sessoes_cargo.append({'numero_sessao': numero_sessao, 'hora_convocacao_inicio': agenda_sessao.get('hora_convocacao_inicio', ''), 'hora_convocacao_fim': agenda_sessao.get('hora_convocacao_fim', ''), 'candidatos': candidatos_base_ordenados})
            ...
            resultado_estruturado = {'processo_uuid': resultado.get('processo_uuid'), 'concurso_uuid': resultado.get('concurso_uuid'), 'todos_processos_uuid': resultado.get('todos_processos_uuid'), 'outros_processos_uuid': resultado.get('outros_processos_uuid'), 'total_cargos': len(cargos_com_sessoes), 'cargos': []}  # type: ignore[var-annotated]
            for cargo_info in cargos_com_sessoes:
            ...
            logger.error('Erro ao processar lauda de convocação (processo_uuid=%s): %s', processo_uuid, exc)
            raise
        except Exception as exc:
            logger.error('Erro inesperado ao processar lauda de convocação (processo_uuid=%s): %s', processo_uuid, exc)
            raise
