"""Módulo services/relatorios/reconvocacao."""
...
class SumulaReconvocacao(RelatorioBase):
    ...
    def gerar(self, processo_uuid: str, request: Any, formato: str='html', cabecalho: str='', **kwargs: Any) -> Any:
        """Gera o relatório de Reconvocados.
        ...
        try:
            escolhas_data = self.escolhas_service.buscar_escolhas_por_candidatos(candidato_uuids=concurso_candidato_uuids, situacao='reconvocacao')
        except Exception as exc:
        ...
        for escolha in escolhas_data:
            ...
            candidatos_com_escolhas.append({'cargo_codigo': cargo_codigo, 'cargo_descricao': cargo_descricao, 'classificacao_geral': classificacao_geral, 'classificacao_def': classificacao_def, 'classificacao_nna': classificacao_nna, 'nome': nome, 'cpf': cpf})
        ...
        if formato == 'xls' or formato == 'csv':
            filename = f'relatorio_reconvocacao_{processo_uuid}.xlsx'
            ...
            filename = f'relatorio_reconvocacao_{processo_uuid}.docx'
            ...
            filename = f'relatorio_reconvocacao_{processo_uuid}.pdf'
    ...
    def _agrupar_por_cargo(self, candidatos: list) -> list:
        ...
        for candidato in candidatos:
            ...
            if cargo_codigo not in cargos_dict:
                cargos_dict[cargo_codigo] = {'codigo': cargo_codigo if cargo_codigo and cargo_codigo != '-' else '', 'descricao': cargo_descricao, 'candidatos': []}
        ...
        for cargo in cargos_list:
            cargo['candidatos'].sort(key=lambda c: c['classificacao_geral'] if isinstance(c['classificacao_geral'], int | float) else float('inf'))
        return cargos_list

    def render_to_xls(self, context: Any=None, filename: Any='relatorio_reconvocacao.xlsx') -> Any:
        ...
        try:
            wb = Workbook()
            ws = wb.active
            ws.title = 'Relatório de Reconvocados'
    ...
    def render_to_docx(self, cargos_list: Any, cabecalho: Any, texto_final: Any, filename: Any='relatorio_reconvocacao.docx') -> Any:
