...
class LaudaConvocacao(RelatorioBase):
    ...
    def gerar(self, processo_uuid: str, request, formato: str = 'html', cabecalho: str = ''):
        ...
        # Por enquanto, retornar apenas os dados em JSON sem gerar relatório
        # Quando necessário gerar HTML/PDF, descomentar o código abaixo
        # logger.info('Retornando dados da lauda de convocação em JSON (sem gerar relatório)')
        # response = JsonResponse(dados_lauda, safe=False)
        # Código comentado para geração de relatório (quando necessário):
        if formato == 'pdf':
