...
class LaudaConvocacao(RelatorioBase):
    ...
    def gerar(self, processo_uuid: str, request: Any, formato: str='html', cabecalho: str='', **kwargs: Any) -> Any:
        ...
        if formato == 'docx' or formato == 'doc':
            ...
            response = self.render_to_docx(dados_lauda.get('cargos', []), self.context, self.context['texto_final'], filename=filename)
            return (response, dados_lauda)
        elif formato == 'pdf':
            filename = f'lauda_convocacao_{processo_uuid}.pdf'
            logger.info('Gerando PDF: %s', filename)
            self.context.update({'is_pdf': True, 'cargos': dados_lauda.get('cargos', [])})
            ...
            response = self._render_xls(dados_lauda.get('cargos', []), context=self.context, filename=filename)
            return (response, dados_lauda)
        elif formato == 'html':
        ...
        for cargo in cargos_list or []:
            ...
            for sessao in sessoes:
                ...
                for r, cand in enumerate(sessao.get('candidatos', []), start=row_idx + 1):
                    values = [cand.get('ordem_escolha'), cand.get('codigo_inscricao') or cand.get('uuid')]
                    nome = ''
                    ...
                    if status_especial:
                        ...
                        if categoria_efetiva == 'NNA' and cand.get('classificacao_nna'):
                            ...
                            nome += ' (PCD)'
                    values.append(nome)
                    values.extend(['-' if str(cand.get('classificacao', '-')) == 'None' else cand.get('classificacao', '-'), '-' if str(cand.get('classificacao_pcd', '-')) == 'None' else cand.get('classificacao_pcd', '-'), '-' if str(cand.get('classificacao_nna', '-')) == 'None' else cand.get('classificacao_nna', '-')])
