    ...
    def _render_xls(self, context: dict[str, Any], filename: str='lista_candidatos_sessao.xlsx') -> HttpResponse:
        ...
        for idx, sec in enumerate(sections):
            ...
            for i, row in enumerate(sec.get('candidatos', []), start=row_idx + 1):
                values = [row.get('classificacao'), row.get('classificacao_nna'), row.get('classificacao_pcd'), row.get('inscricao'), row.get('nome'), row.get('cpf')]
        ...
        if context.get('texto_final'):
            ...
            cell = ws[f'A{row_idx}']
            cell.value = self.processar_cabecalho_html(context.get('texto_final'))  # type: ignore[arg-type]
            cell.font = normal_font
    ...
    def _render_docx(self, context: dict[str, Any], filename: str='lista_candidatos_sessao.docx') -> HttpResponse:
        ...
        for idx, sec in enumerate(sections_list):
            ...
            for i, row in enumerate(sec.get('candidatos', []), start=1):
                ...
                values = [str(row.get('classificacao') or ''), str(row.get('classificacao_nna') or ''), str(row.get('classificacao_pcd') or ''), str(row.get('inscricao') or ''), str(row.get('nome') or ''), str(row.get('cpf') or '')]
        ...
        if context.get('texto_final'):
            ...
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT
            run = p.add_run(self.processar_cabecalho_html(context.get('texto_final')))  # type: ignore[arg-type]
            run.font.size = Pt(10)
            doc.add_paragraph()
        import io
        buf = io.BytesIO()
    ...
    def gerar(self, processo_uuid: str, request: Any, formato: str='html', cabecalho: str='', agenda_uuid: str='', **kwargs: Any) -> tuple[HttpResponse, dict[str, Any]]:  # type: ignore[override]
        ...
        try:
            ...
            if not sections:
                ...
                context = {'agendas': sections, 'agenda': sections[0]['agenda'], 'candidatos': sections[0]['candidatos']}
            else:
