...
class AtaEscolha(RelatorioBase):
    ...
    def gerar(self, processo_uuid: str, request: Any, formato: str='html', cabecalho: str='', cargo_codigo: str=None, **kwargs: Any) -> Any:  # type: ignore[assignment]
        ...
        datas_preencher_tempalte = {'cargo': primeiro_cargo.get('cargo_nome', ''), 'tipos_vagas': 'PRECÁRIAS/DEFINITIVAS'}
        ...
        context_data.update({'cargos': dados_ata.get('cargos', []), 'candidatos_sep_cargo': dados_ata.get('candidatos_sep_cargo', {}), 'cabecalho_capa_ata': cabecalho_capa_ata, 'mostrar_capa_ata': False, 'escolhas_totais_por_tipo': dados_ata.get('escolhas_totais_por_tipo', {}), 'logo_url': logo_url, 'is_pdf': False, 'intervalos_classificacoes': dados_ata.get('intervalos_classificacoes', {}), 'processo_nome': dados_ata.get('processo_nome', '')})
    ...
    def render_to_docx(self, cargos_list: Any, context: Any, filename: Any='ata_escolha.docx') -> Any:
        ...
        try:
            ...
            for cargo in cargos_list:
                ...
                for sessao in cargo.get('sessoes', []):
                    ...
                    shading_elm.set(qn('w:val'), 'clear')
                    p_pr.append(shading_elm)
                    headers = ['Class. Geral', 'Class. Def.', 'Class. NNA', 'RF', 'RG', 'CPF', 'Nome', 'Código EOL', 'DRE', 'Tipo Unidade', 'Unidade Escolhida', 'Tipo Vaga', 'Assinatura']
    ...
    def _render_xls(self, context_data: Any, filename: Any='ata_escolha.xlsx') -> Any:
        ...
        for cargo in context_data.get('cargos', []) or []:
            ...
            for sessao in sessoes:
                ...
                for r, cand in enumerate(sessao.get('candidatos', []), start=row_idx + 1):
                    ...
                    values = ['-' if str(cand.get('classificacao', '-')) == 'None' else str(cand.get('classificacao', '-')), '-' if str(cand.get('classificacao_pcd', '-')) == 'None' else str(cand.get('classificacao_pcd', '-')), '-' if str(cand.get('classificacao_nna', '-')) == 'None' else str(cand.get('classificacao_nna', '-'))]
                    ...
                    values.append(nome)
                    values.extend([str(cand.get('codigo_eol', '-') or '-'), str(cand.get('dre_codigo', '-') or '-'), str(cand.get('tipo_unidade', '-') or '-'), str(cand.get('nome_escola_escolhida', '-') or '-'), str(cand.get('tipo_vaga', '-') or '-'), str(cand.get('assinatura', 'Não Escolha') or 'Não Escolha')])
        ...
        column_widths = {'A': 12, 'B': 12, 'C': 12, 'D': 12, 'E': 15, 'F': 15, 'G': 35, 'H': 12, 'I': 10, 'J': 15, 'K': 40, 'L': 10, 'M': 12}
        ...
        for p in temp_image_paths:
            try:
                ...
                pass
        resp = HttpResponse(buf.read(), content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')  # type: ignore[assignment]
