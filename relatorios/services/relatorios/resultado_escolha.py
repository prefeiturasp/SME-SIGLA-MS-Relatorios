...
class ResultadoEscolha(RelatorioBase):
    ...
    def gerar(self, processo_uuid: str, request: Any, formato: str='html', cabecalho: str='', agenda_uuid: str=None, **kwargs: Any) -> Any:  # type: ignore[assignment]
        ...
        cargos_map = {}
        try:
            cargos_response = self.processos_service.buscar_cargos_por_processo(processo_uuid=str(processo_uuid) if processo_uuid else '')
        ...
        try:
            agendas_response = self.agendas_service.buscar_agendas(processo_convocacao_uuid=str(processo_uuid) if processo_uuid else '', page=1, page_size=1000)
        ...
        if self.tipo == 'RESULTADO_ESCOLHA':
            ...
            for tipo_escolha in tipos_escolha:
                try:
                    escolhas_data = self.escolhas_service.buscar_escolhas_por_candidatos(candidato_uuids=concurso_candidato_uuids, situacao=tipo_escolha)
            ...
            tipo_escolha = 'escolha' if self.tipo == 'RESULTADO_ESCOLHA_SIM' else 'nao-escolha' if self.tipo == 'RESULTADO_ESCOLHA_NAO' else 'reconvocacao' if self.tipo == 'RESULTADO_ESCOLHA_RECONVOCACAO' else None  # type: ignore[assignment]
            try:
                escolhas_data = self.escolhas_service.buscar_escolhas_por_candidatos(candidato_uuids=concurso_candidato_uuids, situacao=tipo_escolha)
            except Exception as exc:
        ...
        for escolha in escolhas_data:
            ...
            if not agenda_data:
                ...
                agenda_data = {'uuid': None, 'cargo_uuid': None, 'cargo_nome': cargo_descricao_candidato, 'cargo_codigo': cargo_codigo_candidato, 'escolha_em': None, 'sessao': None}
            ...
            if self.tipo == 'RESULTADO_ESCOLHA':
                ...
                tipo_escolha_nome = 'Outros'
                escolha_valor = '-'
            item_escolha = {'cargo_codigo': cargo_codigo, 'cargo_descricao': cargo_descricao, 'tipo_escolha': tipo_escolha_nome, 'tipo_escolha_ordem': 1 if tipo_escolha_nome == 'Escolha' else 2 if tipo_escolha_nome == 'Não Escolha' else 3 if tipo_escolha_nome == 'Reconvocação' else 4, 'agenda_uuid': agenda_data.get('uuid'), 'agenda_nome': agenda_data.get('cargo_nome') or cargo_descricao, 'agenda_data': agenda_data.get('escolha_em') or '-', 'agenda_sessao': agenda_data.get('sessao') or '-', 'classificacao_geral': classificacao_geral, 'classificacao_def': classificacao_def, 'classificacao_nna': classificacao_nna, 'nome': nome, 'rg': rg, 'cpf': cpf, 'escolha': escolha_valor}
        ...
        self.context.update({'cargos': cargos_list, 'logo_url': logo_url, 'data_atual': data_atual, 'is_pdf': False})
        if formato == 'xls' or formato == 'csv':
            filename = f'resultado_escolha_{processo_uuid}.xlsx'
            logger.info('Gerando Excel: %s', filename)
            response = self.render_to_xls(cargos_list, self.context.get('cabecalho_padrao'), self.context.get('cabecalho'), filename=filename)
            ...
            response = self.render_to_docx(cargos_list, self.context.get('cabecalho_padrao'), self.context.get('cabecalho'), self.context.get('texto_final'), filename=filename)  # type: ignore[misc]
            return (response, cargos_list)
        elif formato == 'pdf':
    ...
    def _extrair_numero_sessao(self, sessao: str) -> str:
        ...
        sessao_limpa = re.sub('^[Ss]ess[ãa]o\\s*', '', sessao_str, flags=re.IGNORECASE)
    ...
    def _agrupar_por_cargo_e_agenda(self, escolhas: list) -> list:
        ...
        for escolha in escolhas:
            ...
            if cargo_codigo not in cargos_dict:
                cargos_dict[cargo_codigo] = {'codigo': cargo_codigo if cargo_codigo and cargo_codigo != '-' else '', 'descricao': cargo_descricao, 'agendas': {}}
            if agenda_chave not in cargos_dict[cargo_codigo]['agendas']:
                cargos_dict[cargo_codigo]['agendas'][agenda_chave] = {'uuid': agenda_uuid, 'nome': agenda_nome, 'data': agenda_data, 'sessao': sessao_numero, 'candidatos': []}
        ...
        for cargo_codigo, cargo_data in cargos_dict.items():
            ...
            for agenda_chave, agenda_data in cargo_data['agendas'].items():
                agenda_data['candidatos'].sort(key=lambda e: e['classificacao_geral'] if isinstance(e['classificacao_geral'], int | float) and e['classificacao_geral'] != '-' else float('inf'))
                agendas_list.append(agenda_data)
            agendas_list.sort(key=lambda a: (a['data'] if a['data'] != '-' else '', int(a['sessao']) if a['sessao'] != '-' and str(a['sessao']).isdigit() else float('inf') if a['sessao'] != '-' else ''))
            cargos_list.append({'codigo': cargo_data['codigo'], 'descricao': cargo_data['descricao'], 'agendas': agendas_list})
    ...
    def _agrupar_por_cargo_tipo_escolha_e_agenda(self, escolhas: list) -> list:
        ...
        for escolha in escolhas:
            ...
            if agenda_uuid:
                ...
                agenda_chave = f'{agenda_nome}_{agenda_data}_{agenda_sessao}'
            if cargo_codigo not in cargos_dict:
                cargos_dict[cargo_codigo] = {'codigo': cargo_codigo if cargo_codigo and cargo_codigo != '-' else '', 'descricao': cargo_descricao, 'tipos_escolha': {}}
            if tipo_escolha not in cargos_dict[cargo_codigo]['tipos_escolha']:
                cargos_dict[cargo_codigo]['tipos_escolha'][tipo_escolha] = {'nome': tipo_escolha, 'ordem': tipo_escolha_ordem, 'agendas': {}}
            if agenda_chave not in cargos_dict[cargo_codigo]['tipos_escolha'][tipo_escolha]['agendas']:
                cargos_dict[cargo_codigo]['tipos_escolha'][tipo_escolha]['agendas'][agenda_chave] = {'uuid': agenda_uuid, 'nome': agenda_nome, 'data': agenda_data, 'sessao': sessao_numero, 'candidatos': []}
            cargos_dict[cargo_codigo]['tipos_escolha'][tipo_escolha]['agendas'][agenda_chave]['candidatos'].append(escolha)
        cargos_list = []
        for cargo_codigo, cargo_data in cargos_dict.items():
            tipos_escolha_list = []
            for _tipo_escolha_nome, tipo_escolha_data in cargo_data['tipos_escolha'].items():
                agendas_list = []
                for agenda_chave, agenda_data in tipo_escolha_data['agendas'].items():
                    agenda_data['candidatos'].sort(key=lambda e: e['classificacao_geral'] if isinstance(e['classificacao_geral'], int | float) and e['classificacao_geral'] != '-' else float('inf'))
                    agendas_list.append(agenda_data)
                agendas_list.sort(key=lambda a: (a['data'] if a['data'] != '-' else '', int(a['sessao']) if a['sessao'] != '-' and str(a['sessao']).isdigit() else float('inf') if a['sessao'] != '-' else ''))
                tipos_escolha_list.append({'nome': tipo_escolha_data['nome'], 'ordem': tipo_escolha_data['ordem'], 'agendas': agendas_list})
            tipos_escolha_list.sort(key=lambda t: t['ordem'])
            cargos_list.append({'codigo': cargo_data['codigo'], 'descricao': cargo_data['descricao'], 'tipos_escolha': tipos_escolha_list})
        cargos_list.sort(key=lambda x: x['descricao'])
        return cargos_list
    ...
    def _adicionar_resumo_dre_escola(self, cargos_list: list, escolhas_com_candidatos: list, processo_uuid: str) -> list:
        ...
        for e in escolhas_realizadas:
            ...
            if escola_chave not in por_cargo[cargo_codigo][dre_codigo]['escolas']:
                por_cargo[cargo_codigo][dre_codigo]['escolas'][escola_chave] = {'nome': escola_nome, 'codigo_eol': escola_eol, 'tipo_ue': tipo_ue, 'qtd_escolhas_definitivas': 0, 'qtd_escolhas_precarias': 0}
        ...
        for cargo in cargos_list:
            ...
            for dre_codigo, dre_info in sorted(dres_data.items(), key=lambda x: (x[1]['nome'], x[0])):
                ...
                for escola_chave, esc_info in sorted(dre_info['escolas'].items(), key=lambda x: (x[1]['nome'], x[0])):
                    ...
                    escolas_list.append({'nome': esc_info['nome'], 'codigo_eol': esc_info.get('codigo_eol', '') or '-', 'tipo_ue': esc_info.get('tipo_ue', '') or '-', 'qtd_vagas_definitivas': vagas.get('definitivas', 0), 'qtd_vagas_precarias': vagas.get('precarias', 0), 'qtd_escolhas_definitivas': esc_info.get('qtd_escolhas_definitivas', 0), 'qtd_escolhas_precarias': esc_info.get('qtd_escolhas_precarias', 0)})
                resumo_dres.append({'nome': dre_info['nome'], 'escolas': escolas_list})
            cargo['resumo_dre_escola'] = resumo_dres
        return cargos_list

    def render_to_xls(self, cargos_list: Any, cabecalho_padrao: Any, cabecalho: Any, filename: Any='resultado_escolha.xlsx') -> Any:
        ...
        try:
            ...
            column_widths = {'A': max(10, escola_width), 'B': 12, 'C': 12, 'D': 14, 'E': 40, 'F': 18, 'G': 16, 'H': 10}
            ...
            if max_escola_length > 0:
                ...
                for col_letter, width in resumo_numeric_widths.items():
                    if col_letter == 'E':
                        ...
                        ws.column_dimensions[col_letter].width = max(current_width, 40)
                    else:
