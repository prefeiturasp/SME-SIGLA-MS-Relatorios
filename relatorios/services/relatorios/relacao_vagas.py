...
class RelacaoVagas(RelatorioBase):
    ...
    def gerar(self, processo_uuid: str, request: Any, formato: str='html', cabecalho: str='', **kwargs: Any) -> Any:
        ...
        self.context.update({'cargos': cargos_list, 'is_pdf': False, 'logo_url': logo_url})
        if formato == 'xls' or formato == 'csv':
            ...
            response = self.render_to_docx(cargos_list, cabecalho_final, self.context['texto_final'], filename=filename)
            return (response, cargos_list)
        elif formato == 'pdf':
                ...
                for dre_codigo, vagas_list in dres.items():
                    if vagas_list:
                        ...
                        for vaga in vagas_list:
                            vagas_definitivas_originais = vaga.get('vagas_definitivas_utilizadas') if vaga.get('vagas_definitivas_utilizadas') is not None else vaga.get('vagas_definitivas', 0) or 0
                            vagas_precarias_originais = vaga.get('vagas_precarias_utilizadas') if vaga.get('vagas_precarias_utilizadas') is not None else vaga.get('vagas_precarias', 0) or 0
                            vagas_definitivas_atuais = vaga.get('vagas_definitivas_restantes', 0) or 0
                            vagas_precarias_atuais = vaga.get('vagas_precarias_restantes', 0) or 0
                            vaga_calculada = vaga.copy()
                            vaga_calculada['vagas_definitivas_originais'] = vagas_definitivas_originais
                            vaga_calculada['vagas_definitivas_atuais'] = vagas_definitivas_atuais
                            vaga_calculada['vagas_precarias_originais'] = vagas_precarias_originais
                            vaga_calculada['vagas_precarias_atuais'] = vagas_precarias_atuais
                            vagas_com_calculos.append(vaga_calculada)
                        dres_list.append({'codigo': dre_codigo, 'nome': primeira_vaga_dre.get('escola', {}).get('dre', {}).get('nome', ''), 'vagas': vagas_com_calculos})
                cargos_list.append({'codigo': cargo_codigo, 'descricao': primeira_vaga.get('cargo_descricao', ''), 'dres': dres_list})
        return cargos_list

    def render_to_xls(self, context: Any=None, filename: Any='relacao_vagas.xlsx') -> Any:
        ...
        try:
            ...
            for cargo in context.get('cargos', []):
                ...
                for dre in cargo.get('dres', []):
                    ...
                    cell.alignment = left_align
                    row += 1
                    headers = ['Tipo de unidade', 'Código EOL', 'Vagas Definitivas Originais', 'Vagas Definitivas Atuais', 'Vagas Precárias Originais', 'Vagas Precárias Atuais']
                    ...
                    for vaga in dre.get('vagas', []):
                        ...
                        ws.cell(row=row, column=2).value = escola.get('codigo_eol', '-')
                        ws.cell(row=row, column=3).value = vaga.get('vagas_definitivas_originais', 0)
                        ws.cell(row=row, column=4).value = vaga.get('vagas_definitivas_atuais', 0)
                        ws.cell(row=row, column=5).value = vaga.get('vagas_precarias_originais', 0)
                        ws.cell(row=row, column=6).value = vaga.get('vagas_precarias_atuais', 0)
                    ...
                    row += 1
                row += 1
            column_widths = {'A': 20, 'B': 20, 'C': 25, 'D': 25, 'E': 25, 'F': 25}
    ...
    def render_to_docx(self, cargos_list: Any, cabecalho: Any, texto_final: Any, filename: Any='relacao_vagas.docx') -> Any:
            ...
            for cargo in cargos_list:
                ...
                for dre in cargo.get('dres', []):
                    ...
                    for vaga in dre.get('vagas', []):
                        ...
                        row_cells[2].text = str(vaga.get('vagas_definitivas_originais', 0))
                        row_cells[3].text = str(vaga.get('vagas_definitivas_atuais', 0))
                        row_cells[4].text = str(vaga.get('vagas_precarias_originais', 0))
                        row_cells[5].text = str(vaga.get('vagas_precarias_atuais', 0))
