                ...
                for dre_codigo, vagas_list in dres.items():
                    if vagas_list:
                        ...
                        for vaga in vagas_list:
                            ...
                            vaga_calculada["vagas_definitivas_atuais"] = (
                                vagas_definitivas_atuais
                            )
                            vaga_calculada["vagas_precarias_originais"] = (
                                vagas_precarias_originais
                            )
                            vaga_calculada["vagas_precarias_atuais"] = (
                                vagas_precarias_atuais
                            )
            ...
            for cargo in context.get("cargos", []):
                ...
                for dre in cargo.get("dres", []):
                    ...
                    headers = [
                        "Tipo de unidade",
                        "Código EOL",
                        "Vagas Definitivas Originais",
                        "Vagas Definitivas Atuais",
                        "Vagas Precárias Originais",
                        "Vagas Precárias Atuais",
                    ]
                    ...
                    for vaga in dre.get("vagas", []):
                        ...
                        ws.cell(row=row, column=3).value = vaga.get(
                            "vagas_definitivas_originais", 0
                        )
                        ws.cell(row=row, column=4).value = vaga.get(
                            "vagas_definitivas_atuais", 0
                        )
                        ws.cell(row=row, column=5).value = vaga.get(
                            "vagas_precarias_originais", 0
                        )
                ...
                for dre in cargo.get("dres", []):
                    ...
                    for vaga in dre.get("vagas", []):
                        ...
                        row_cells[2].text = str(
                            vaga.get("vagas_definitivas_originais", 0)
                        )
                        row_cells[3].text = str(
                            vaga.get("vagas_definitivas_atuais", 0)
                        )
                        row_cells[4].text = str(
                            vaga.get("vagas_precarias_originais", 0)
                        )
