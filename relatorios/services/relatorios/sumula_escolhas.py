...
class SumulaEscolhas(RelatorioBase):
    ...
    def gerar(self, processo_uuid: str, request: Any, formato: str='html', cabecalho: str='', **kwargs: Any) -> Any:
        ...
        try:
            escolhas_data = self.escolhas_service.buscar_escolhas_por_candidatos(candidato_uuids=concurso_candidato_uuids, situacao=None)  # type: ignore[arg-type]
        except Exception as exc:
            logger.error('Falha ao buscar escolhas da API externa: %s', exc)
            raise
        escolhas_realizadas = [e for e in escolhas_data if e.get('situacao') not in ['nao-escolha', 'reconvocacao', None]]
        ...
        for escolha in escolhas_realizadas:
            ...
            if tipo_vaga_raw == 'precaria':
                ...
                tipo_vaga = '-'
            escolhas_com_candidatos.append({'cargo_codigo': cargo_codigo, 'cargo_descricao': cargo_descricao, 'dre_codigo': dre_codigo, 'dre_nome': dre_nome, 'escola_nome': nome_oficial, 'escola_codigo_eol': codigo_eol, 'classificacao': classificacao_coluna_geral, 'classificacao_nna': classificacao_coluna_nna, 'classificacao_pcd': classificacao_coluna_pcd, 'classificacao_ordem': classificacao_ordem, 'nome_candidato': nome_formatado, 'tipo_vaga': tipo_vaga})
        ...
        if formato == 'xls' or formato == 'csv':
            ...
            response = self.render_to_docx(cargos_list, cabecalho_docx, filename=filename)
            return (response, cargos_list)
        elif formato == 'pdf':
    ...
    def _agrupar_por_cargo_dre_e_escola(self, escolhas: list) -> list:
        ...
        for escolha in escolhas:
            ...
            if cargo_codigo not in cargos_dict:
                cargos_dict[cargo_codigo] = {'codigo': cargo_codigo if cargo_codigo and cargo_codigo != '-' else '', 'descricao': cargo_descricao, 'dres': {}}
            if dre_codigo not in cargos_dict[cargo_codigo]['dres']:
                cargos_dict[cargo_codigo]['dres'][dre_codigo] = {'codigo': dre_codigo if dre_codigo else '', 'nome': dre_nome, 'escolas': {}}
            if escola_chave not in cargos_dict[cargo_codigo]['dres'][dre_codigo]['escolas']:
                cargos_dict[cargo_codigo]['dres'][dre_codigo]['escolas'][escola_chave] = {'nome': escola_nome, 'codigo_eol': escola_codigo_eol, 'escolhas': []}
        ...
        for cargo_codigo, cargo_data in cargos_dict.items():
            ...
            for dre_codigo, dre_data in cargo_data['dres'].items():
                ...
                for escola_chave, escola_data in dre_data['escolas'].items():
                    escola_data['escolhas'].sort(key=lambda e: e.get('classificacao_ordem') if isinstance(e.get('classificacao_ordem'), int | float) else e.get('classificacao') if isinstance(e.get('classificacao'), int | float) else float('inf'))
                    escolas_list.append(escola_data)
                escolas_list.sort(key=lambda e: e['nome'])
                dres_list.append({'codigo': dre_data['codigo'], 'nome': dre_data['nome'], 'escolas': escolas_list})
            dres_list.sort(key=lambda d: d['nome'])
            cargos_list.append({'codigo': cargo_data['codigo'], 'descricao': cargo_data['descricao'], 'dres': dres_list})
        cargos_list.sort(key=lambda x: x['descricao'])
        return cargos_list

    def render_to_xls(self, context: Any=None, filename: Any='relatorio_sumula_escolhas.xlsx') -> Any:
    ...
    def render_to_docx(self, cargos_list: Any, cabecalho: Any, filename: Any='relatorio_sumula_escolhas.docx') -> Any:
        """Gera um arquivo Word (DOCX) mantendo a estrutura hierárquica do Excel.

        Args:
            cargos_list: Lista de cargos com suas DREs e escolhas (estrutura
            hierárquica)
