...
class EscolhasService:
    ...
    def buscar_vagas_escolas(self, processo_uuid: str) -> requests.Response:
        ...
        logger.info('Buscando vagas de escolas', extra={'correlation_id': get_correlation_id(), 'method': 'GET', 'processo_uuid': processo_uuid, 'url': url, 'headers': self._default_headers, 'params': params})
        try:
            ...
            raise
        logger.info('Vagas de escolas encontradas', extra={'correlation_id': get_correlation_id(), 'method': 'GET', 'processo_uuid': processo_uuid, 'url': url, 'headers': self._default_headers, 'params': params, 'status_code': response.status_code, 'response': str(response.json())[:100]})
    ...
    def buscar_escolhas_por_candidatos(self, candidato_uuids: list, situacao: str='nao-escolha') -> list:
        ...
        logger.info('Buscando escolhas por candidatos', extra={'correlation_id': get_correlation_id(), 'method': 'POST', 'candidato_uuids': candidato_uuids, 'situacao': situacao, 'url': url, 'headers': self._default_headers})
        try:
            response = http_client.post(url, json=data, headers=self._default_headers, timeout=self.timeout_seconds)
            response.raise_for_status()
        except RequestException as exc:
        ...
        logger.info('Escolhas buscadas com sucesso (candidatos=%d, situacao=%s, filtradas=%d)', len(candidato_uuids), situacao, len(escolhas_filtradas))
