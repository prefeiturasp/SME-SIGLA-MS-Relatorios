...
class RelatorioViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if format_param == 'xls' or format_param == 'xlsx' or 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' in accept_header:
            ...
            formato = 'html'
        logger.info('Tipo de relatório: %s, Formato solicitado: %s', tipo_relatorio, formato)
        try:
            relatorio_service = RelatorioFactory.obter_relatorio(tipo_relatorio)
            response, dados = relatorio_service.gerar(processo_uuid, request, formato, agenda_uuid=agenda_uuid, cargo_codigo=cargo_codigo)
            try:
            ...
            return Response({'error': exc.message, 'cargos': exc.cargos}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({'error': f'Erro ao gerar relatório: {str(exc)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
