"""Módulo tests/services/test_ata_escolha_service."""
...
def test_processar_ata_escolha_um_cargo_quando_informa_cargo_codigo(service: Any) -> None:
    ...
    service.candidatos_service.buscar_por_uuids.return_value = _Resp({'results': [{'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1, 'candidato': {'nome': 'Candidato A'}}]})
    service.candidatos_service.buscar_reclassificados_por_concurso.return_value = _Resp({})
    service.candidatos_service.buscar_eliminados_por_concurso.return_value = _Resp({})
