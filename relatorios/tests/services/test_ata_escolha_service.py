...
def test_separar_por_tipo(service):
    ...
    assert separados == {
        ...
        'pcd': [{'categoria_efetiva': 'PCD', 'uuid': 'p1'}, {'categoria_efetiva': 'PCD', 'uuid': 'p2'}]
    }


@pytest.mark.parametrize('candidatos,campo,esperado', [
    ([{'classificacao': 1}, {'classificacao': None}, {}, {'classificacao': 5}], 'classificacao', [1, None, None, 5]),
    ([{'classificacao_pcd': 2}, {'classificacao_pcd': None}], 'classificacao_pcd', [2, None]),
])
...
def test_buscar_candidatos_faltantes_sucesso(service):
    ...
    service.candidatos_service.buscar_habilitados_por_processos_e_classificacoes.side_effect = [
    ...
    service.candidatos_service.buscar_habilitados.side_effect = [
        _Resp([{'uuid': 'p1', 'classificacao_pcd': 2}, {'uuid': 'z', 'classificacao_pcd': None}]),
    ...
    assert res['geral'][0]['status_especial'] in ['JÁ CONVOCADO - LEI 13.398/02', 'JÁ CONVOCADO - LEI 15.939/13', '']


@pytest.mark.parametrize('outros_processos,esperado', [
    ([], {'geral': [], 'nna': [], 'pcd': []}),
])
def test_buscar_candidatos_faltantes_sem_outros_processos(service, outros_processos, esperado):
...
@pytest.mark.parametrize('candidato_uuids,escolhas_data,esperado_len,esperado_uuids', [
    (['uuid1', 'uuid2', 'uuid3'], [
        {'candidato_uuid': 'uuid1', 'situacao': 'escolha', 'tipo_vaga': 'precaria'},
        {'candidato_uuid': 'uuid2', 'situacao': 'escolha', 'tipo_vaga': 'definitiva'},
    ], 2, ['uuid1', 'uuid2']),
    ([], [], 0, []),
])
def test_buscar_escolhas_por_candidatos(service, candidato_uuids, escolhas_data, esperado_len, esperado_uuids):
...
@pytest.mark.parametrize('escolha,esperado', [
    ({
        'tipo_vaga': 'precaria',
        'vaga_escola': {
            'escola': {
                ...
                'tipo_ue': 'EMEF',
                'dre': {'sigla': 'DRE-TEST', 'nome': 'Diretoria Regional de Teste'}
            }
        }
    }, {
        'codigo_eol': '12345',
...
def test_processar_ata_escolha_fluxo_basico(service, agenda_basica, candidato_basico):
    ...
    agendas = [
        ...
        {**agenda_basica, 'candidatos_uuids': ['b', 'c'], 'hora_convocacao_inicio': '10:00', 'hora_convocacao_fim': '11:00'}
    ]
    candidatos = [
        {**candidato_basico, 'uuid': 'a', 'candidato': {'nome': 'Candidato A', 'rg': '123456', 'cpf': '11111111111', 'registro_funcional': 'RF001'}},
        {**candidato_basico, 'uuid': 'b', 'classificacao': 2, 'ranking_escolha': 2, 'candidato': {'nome': 'Candidato B', 'rg': '234567', 'cpf': '22222222222', 'registro_funcional': 'RF002'}},
        {**candidato_basico, 'uuid': 'c', 'classificacao': 3, 'ranking_escolha': 3, 'candidato': {'nome': 'Candidato C', 'rg': '345678', 'cpf': '33333333333', 'registro_funcional': 'RF003'}},
    ]
    escolhas = [{
        'candidato_uuid': 'a',
        'situacao': 'escolha',
    ...
    candidatos_todos = [c for sessao in cargo['sessoes'] for c in sessao['candidatos']]
...
def test_processar_ata_escolha_com_lacunas_e_faltantes(service, agenda_basica):
    ...
    candidatos = [
        ...
        {'uuid': 'b', 'categoria_efetiva': 'GERAL', 'classificacao': 3, 'ranking_escolha': 2, 'candidato': {'nome': 'Candidato B'}},
    ]
    ...
    service.candidatos_service.buscar_habilitados_por_processos_e_classificacoes.return_value = _Resp([
        {'uuid': 'gx', 'categoria_efetiva': 'GERAL', 'classificacao': 2, 'candidato': {'nome': 'Faltante GX'}}
    ])

    resultado = service.processar_ata_escolha(processo_uuid='proc-123')
    candidatos_todos = [c for sessao in resultado['cargos'][0]['sessoes'] for c in sessao['candidatos']]
...
def test_processar_ata_escolha_ordenacao_pcd_primeiro(service, agenda_basica):
    ...
    candidatos = [
        {'uuid': 'geral1', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1, 'candidato': {'nome': 'Geral 1'}},
        {'uuid': 'pcd1', 'categoria_efetiva': 'PCD', 'classificacao_pcd': 1, 'ranking_escolha': 5, 'candidato': {'nome': 'PCD 1'}},
        {'uuid': 'nna1', 'categoria_efetiva': 'NNA', 'classificacao_nna': 1, 'ranking_escolha': 2, 'candidato': {'nome': 'NNA 1'}},
    ]
...
def test_processar_ata_escolha_ordem_escolha(service, agenda_basica):
    ...
    candidatos = [
        {'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1, 'candidato': {'nome': 'Candidato A'}},
        {'uuid': 'b', 'categoria_efetiva': 'GERAL', 'classificacao': 2, 'ranking_escolha': 2, 'candidato': {'nome': 'Candidato B'}},
    ]
    _setup_processamento_basico(service, [agenda_basica], candidatos)
...
def test_processar_ata_escolha_ordem_escolha_com_status_especial(service, agenda_basica):
    """Testa que candidatos com status_especial não recebem ordem_escolha."""
    candidatos = [
        {'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1, 'candidato': {'nome': 'Candidato A'}},
        {'uuid': 'nna1', 'categoria_efetiva': 'NNA', 'classificacao_nna': 1, 'ranking_escolha': 2, 'candidato': {'nome': 'NNA 1'}},
        {'uuid': 'nna3', 'categoria_efetiva': 'NNA', 'classificacao_nna': 3, 'ranking_escolha': 3, 'candidato': {'nome': 'NNA 3'}},
    ]
    _setup_processamento_basico(service, [agenda_basica], candidatos, processo_data={'concurso_uuid': 'cu1'})
    service.candidatos_service.buscar_habilitados_por_processos_e_classificacoes.side_effect = lambda **kwargs: (
        _Resp([{'uuid': 'faltante_nna', 'categoria_efetiva': 'NNA', 'classificacao_nna': 2, 'classificacao': 99, 'candidato': {'nome': 'Faltante NNA'}}])
        if 'classificacao_nna' in kwargs else _Resp([])
    )

    resultado = service.processar_ata_escolha(processo_uuid='proc-status')
    candidatos_todos = [c for sessao in resultado['cargos'][0]['sessoes'] for c in sessao['candidatos']]

    candidato_faltante = next(c for c in candidatos_todos if c['uuid'] == 'faltante_nna')
    assert candidato_faltante.get('ordem_escolha') is None
    assert candidato_faltante.get('status_especial') == 'CANDIDATOS JÁ CLASSIFICADO.'
...
@pytest.mark.parametrize('agendas_payload,candidatos_payload', [
    ({'results': [{'cargo_nome': 'Professor', 'cargo_codigo': '123', 'candidatos_uuids': [], 'hora_convocacao_inicio': '09:00', 'hora_convocacao_fim': '10:00'}]},
     {'results': [{'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1, 'candidato': {'nome': 'Candidato A'}}]}),
    ([{'cargo_nome': 'Professor', 'cargo_codigo': '123', 'candidatos_uuids': [], 'hora_convocacao_inicio': '09:00', 'hora_convocacao_fim': '10:00'}],
     [{'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1, 'candidato': {'nome': 'Candidato A'}}]),
])
...
@pytest.mark.parametrize('hora_inicio,hora_fim,esperado', [
    ('09:00', '10:00', '09:00 às 10:00'),
    ('', '', 'Não informado'),
])
def test_processar_ata_escolha_horario_formatado(service, hora_inicio, hora_fim, esperado):
    ...
    assert resultado['cargos'][0]['sessoes'][0]['horario_formatado'] == esperado
