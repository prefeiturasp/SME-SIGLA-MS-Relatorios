...
def test_buscar_candidatos_faltantes_sucesso() -> None:
    ...
    lac_nna = [3]
    lac_pcd = [2]
    svc.candidatos_service.buscar_habilitados_por_processos_e_classificacoes.side_effect = [_Resp([{'uuid': 'gX', 'classificacao': 4}, {'uuid': 'gY', 'classificacao': 5}]), _Resp([{'uuid': 'nX', 'classificacao_nna': 3}])]
    svc.candidatos_service.buscar_habilitados.side_effect = [_Resp([{'uuid': 'p1', 'classificacao_pcd': 2}, {'uuid': 'z', 'classificacao_pcd': None}]), _Resp([{'uuid': 'p2', 'classificacao_pcd': 99}])]
    res = svc._buscar_candidatos_faltantes(outros_processos_uuid=outros, lacunas_geral=lac_geral, lacunas_nna=lac_nna, lacunas_pcd=lac_pcd, codigo_cargo='123', ordering='ranking_escolha')
...
def test_processar_lauda_convocacao_fluxo_basico() -> None:
    ...
    candidatos_payload = [{'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1}, {'uuid': 'b', 'categoria_efetiva': 'GERAL', 'classificacao': 2, 'ranking_escolha': 2}, {'uuid': 'c', 'categoria_efetiva': 'GERAL', 'classificacao': 3, 'ranking_escolha': 3}]
...
def test_processar_lauda_convocacao_dict_results_merges_geral_faltantes() -> None:
    ...
    svc.agendas_service.buscar_agendas.return_value = _Resp(agendas_payload)
    candidatos_payload = {'results': [{'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1}, {'uuid': 'b', 'categoria_efetiva': 'GERAL', 'classificacao': 3, 'ranking_escolha': 2}, {'uuid': 'c', 'categoria_efetiva': 'GERAL', 'classificacao': 4, 'ranking_escolha': 3}]}
    ...
    svc._buscar_candidatos_faltantes = Mock(return_value={'geral': [{'uuid': 'gx', 'categoria_efetiva': 'GERAL', 'classificacao': 3}, {'uuid': 'gy', 'categoria_efetiva': 'GERAL', 'classificacao': 2}], 'nna': [], 'pcd': []})
...
def test_processar_lauda_convocacao_sorts_and_merges_all_categories() -> None:
    ...
    svc.agendas_service.buscar_agendas.return_value = _Resp(agendas_payload)
    candidatos_payload = {'results': [{'uuid': 'a', 'categoria_efetiva': 'GERAL', 'classificacao': 1, 'ranking_escolha': 1}, {'uuid': 'b', 'categoria_efetiva': 'GERAL', 'classificacao': 4, 'ranking_escolha': 2}, {'uuid': 'n1', 'categoria_efetiva': 'NNA', 'classificacao_nna': 1}, {'uuid': 'n2', 'categoria_efetiva': 'NNA', 'classificacao_nna': 3}, {'uuid': 'p1', 'categoria_efetiva': 'PCD', 'classificacao_pcd': 5}, {'uuid': 'p2', 'categoria_efetiva': 'PCD', 'classificacao_pcd': 7}]}
    svc.candidatos_service.buscar_por_uuids.return_value = _Resp(candidatos_payload)
    svc.candidatos_service.buscar_reclassificados_por_concurso.return_value = _Resp({})
    svc.candidatos_service.buscar_eliminados_por_concurso.return_value = _Resp({})
    ...
    svc._buscar_candidatos_faltantes = Mock(return_value={'geral': [{'uuid': 'g3', 'categoria_efetiva': 'GERAL', 'classificacao': 3}, {'uuid': 'g2', 'categoria_efetiva': 'GERAL', 'classificacao': 2}], 'nna': [{'uuid': 'nn3', 'categoria_efetiva': 'NNA', 'classificacao_nna': 3, 'classificacao': 8}, {'uuid': 'nn2', 'categoria_efetiva': 'NNA', 'classificacao_nna': 2, 'classificacao': 6}], 'pcd': [{'uuid': 'pp7', 'categoria_efetiva': 'PCD', 'classificacao_pcd': 7, 'classificacao': 9}, {'uuid': 'pp6', 'categoria_efetiva': 'PCD', 'classificacao_pcd': 6, 'classificacao': 7}]})
    resultado = svc.processar_lauda_convocacao(processo_uuid=processo_uuid)
    cargo = resultado['cargos'][0]
    all_uuids = [c['uuid'] for s in cargo['sessoes'] for c in s['candidatos']]
