...
def _make_cargo_list(**kwargs):
    ...
    cargo_base = {
        ...
        'sessoes': kwargs.get('sessoes', [{
            'numero_sessao': 1,
            'horario_formatado': kwargs.get('horario_formatado', '09:00 às 10:00'),
            'candidatos': kwargs.get('candidatos', [])
        }])
    }
    return [cargo_base] if not kwargs.get('multiplos_cargos') else [
        cargo_base,
        {
...
@pytest.mark.parametrize('formato,cabecalho,expected_method,expected_content_type,expected_filename', [
    ('html', 'CABECALHO_TESTE', 'render', None, None),
    ('pdf', 'CABECALHO', 'render_to_pdf', 'application/pdf', 'ata_escolha_proc-123.pdf'),
    ('docx', 'CABECALHO', 'render_to_docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'ata_escolha_proc-123.docx'),
    ('doc', 'CABECALHO', 'render_to_docx', None, None),
    ('xlsx', 'CABECALHO', '_render_xls', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'ata_escolha_proc-123.xlsx'),
    ('xls', 'CABECALHO', '_render_xls', None, None),
    ('json', 'CABECALHO', None, None, None),
])
def test_gerar_formatos(settings_config, service_mocked, dados_ata, request_obj, formato, cabecalho, expected_method, expected_content_type, expected_filename):
...
def test_gerar_processo_uuid_none(settings_config, service_mocked, dados_ata, request_obj):
...
@pytest.mark.parametrize('cabecalho,cargos_list', [
    ('CABECALHO TESTE', None),  # None usa dados_ata['cargos']
    ('', None),
    ('CABECALHO', {'candidatos': [{'classificacao': 1, 'status_especial': 'CANDIDATOS JÁ CLASSIFICADO.', 'codigo_eol': '', 'dre_codigo': '', 'tipo_unidade': '', 'nome_escola_escolhida': '', 'tipo_vaga': '', 'assinatura': 'Não Escolha', 'candidato': {}}]}),
    ('CABECALHO', {'candidatos': [{'classificacao': 1, 'nome': 'Nome Direto', 'rf': 'RF001', 'rg': '123456', 'cpf': '11111111111', 'codigo_eol': '12345', 'dre_codigo': 'DRE-A', 'tipo_unidade': 'EMEF', 'nome_escola_escolhida': 'Escola', 'tipo_vaga': 'P', 'assinatura': 'Escolha'}]}),
    ('CABECALHO', {'candidatos': [{'classificacao': None, 'classificacao_pcd': None, 'classificacao_nna': None, 'nome': 'Candidato', 'rf': 'RF001', 'rg': '123456', 'cpf': '11111111111', 'codigo_eol': '', 'dre_codigo': '', 'tipo_unidade': '', 'nome_escola_escolhida': '', 'tipo_vaga': '', 'assinatura': 'Não Escolha', 'candidato': {}}]}),
    ('CABECALHO', {'multiplos_cargos': True, 'candidatos': [{'classificacao': 1, 'nome': 'Candidato 1', 'rf': 'RF001', 'rg': '123456', 'cpf': '11111111111', 'codigo_eol': '', 'dre_codigo': '', 'tipo_unidade': '', 'nome_escola_escolhida': '', 'tipo_vaga': '', 'assinatura': 'Não Escolha', 'candidato': {}}]}),
...
def test_render_to_docx_variacoes(settings_config, service, dados_ata, cabecalho, cargos_list):
    ...
    if cabecalho == 'CABECALHO TESTE':
        ...
        assert 'attachment; filename="test.docx"' in response['Content-Disposition']
...
@pytest.mark.parametrize('cabecalho,cargos_list,check_content_type', [
    ('CABECALHO TESTE', None, True),
    ('', None, False),
    ('CABECALHO', {'candidatos': [{'classificacao': 1, 'status_especial': 'CANDIDATOS JÁ CLASSIFICADO.', 'codigo_eol': '', 'dre_codigo': '', 'tipo_unidade': '', 'nome_escola_escolhida': '', 'tipo_vaga': '', 'assinatura': 'Não Escolha', 'candidato': {}}]}, False),
    ('CABECALHO', {'candidatos': [{'classificacao': 1, 'nome': 'Nome Direto', 'rf': 'RF001', 'rg': '123456', 'cpf': '11111111111', 'codigo_eol': '12345', 'dre_codigo': 'DRE-A', 'tipo_unidade': 'EMEF', 'nome_escola_escolhida': 'Escola', 'tipo_vaga': 'P', 'assinatura': 'Escolha'}]}, False),
    ('CABECALHO', {'candidatos': [{'classificacao': None, 'classificacao_pcd': None, 'classificacao_nna': None, 'nome': 'Candidato', 'rf': 'RF001', 'rg': '123456', 'cpf': '11111111111', 'codigo_eol': '', 'dre_codigo': '', 'tipo_unidade': '', 'nome_escola_escolhida': '', 'tipo_vaga': '', 'assinatura': 'Não Escolha', 'candidato': {}}]}, False),
    ('CABECALHO', {'multiplos_cargos': True, 'candidatos': [{'classificacao': 1, 'nome': 'Candidato 1', 'rf': 'RF001', 'rg': '123456', 'cpf': '11111111111', 'codigo_eol': '', 'dre_codigo': '', 'tipo_unidade': '', 'nome_escola_escolhida': '', 'tipo_vaga': '', 'assinatura': 'Não Escolha', 'candidato': {}}]}, False),
...
def test_render_xls_variacoes(settings_config, service, dados_ata, cabecalho, cargos_list, check_content_type):
    """Testa geração de XLSX com diferentes variações de dados."""
    if cargos_list == 'empty_list':
        cargos = []
    elif cargos_list == {'sessoes': []}:
        cargos = [{'cargo_nome': 'Professor', 'cargo_codigo': '123', 'sessoes': []}]
...
@pytest.mark.skipif(not DOCX_AVAILABLE, reason="python-docx não está instalado")
def test_render_to_docx_exception_handling(settings_config, service, dados_ata):
