...
from relatorios.services.relatorios.ata_escolha import DOCX_AVAILABLE, OPENPYXL_AVAILABLE, AtaEscolha
...
def test_gerar_formatos(settings_config: Any, service_mocked: Any, dados_ata: Any, request_obj: Any, formato: Any, cabecalho: Any, expected_method: Any, expected_content_type: Any, expected_filename: Any) -> None:
    ...
    if formato == 'html':
        ...
        with patch.object(service_mocked, 'render_to_docx', return_value=mock_response) as m_docx:
            response, dados = service_mocked.gerar(processo_uuid='proc-123', request=request_obj, formato=formato, cabecalho=cabecalho)
...
def test_gerar_processo_uuid_none(settings_config: Any, service_mocked: Any, dados_ata: Any, request_obj: Any) -> None:
    ...
    service_mocked.ata_service.processar_ata_escolha.assert_called_once_with(processo_uuid='', cargo_codigo=None)
