...
def test_gerar_pdf_success(settings, configuracao_relatorio, parametrizacao):
    ...
    svc.lauda_service.processar_lauda_convocacao.return_value = {
        "cargos": [{"cargo_nome": "Professor"}]
    }

    with patch.object(
        svc,
        "render_to_pdf",
        return_value=HttpResponse(b"%PDF-1.4", content_type="application/pdf"),
    ) as m_pdf:
        response, dados = svc.gerar(
            processo_uuid="abc",
...
def test_gerar_html_calls_render_with_context_and_default_header(
    ...
    with patch(
        "relatorios.services.relatorios.lauda_convocacao.render",
    ...
    assert isinstance(response, HttpResponse)
    # Verifica que o contexto enviado para render inclui o cabecalho_padrao da Parametrizacao  # noqa: E501
...
def test_gerar_default_json_success(
    settings, configuracao_relatorio, parametrizacao
):
    settings.CANDIDATOS_API_URL = "http://candidatos"
    settings.CONVOCACAO_API_URL = "http://processos"
...
def test_gerar_raises_exception_on_service_failure(
    ...
    svc.lauda_service = Mock()
    svc.lauda_service.processar_lauda_convocacao.side_effect = Exception(
        "falha"
    )
