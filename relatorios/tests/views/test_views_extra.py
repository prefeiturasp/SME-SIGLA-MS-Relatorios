...
def _payload(tipo="LISTA_CANDIDATOS_SESSAO"):
    return {
        "tipo": tipo,
        "usuario": "u1",
...
def test_create_returns_xls_when_query_param_is_xls(mock_factory, client):
    svc = Mock()
...
@patch("relatorios.views.relatorios.RelatorioFactory.obter_relatorio")
def test_create_returns_pdf_when_query_param_pdf(mock_factory, client):
    svc = Mock()
    svc.gerar.return_value = (HttpResponse("ok"), {"ok": True})
    mock_factory.return_value = svc
...
@patch("relatorios.views.relatorios.RelatorioFactory.obter_relatorio")
def test_create_returns_docx_when_query_param_docx(mock_factory, client):
    svc = Mock()
    svc.gerar.return_value = (HttpResponse("ok"), {"ok": True})
    mock_factory.return_value = svc
...
@patch("relatorios.views.relatorios.RelatorioFactory.obter_relatorio")
def test_create_handles_generic_exception(mock_factory, client):
    svc = Mock()
